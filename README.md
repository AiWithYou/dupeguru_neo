...
- **明示的な永続ライブラリカタログ。** CLI で選んだ場合だけ、ローカル SQLite
  カタログが安定したファイル ID、パス、内容世代、完全一致の派生成果物、変更不能な
  スキャン履歴、再開可能な作業を保持します。画像特徴、サムネイル、動画
  フィンガープリント、ファイル操作の復旧ジャーナルは、検証規則とライフサイクルを
  分離した別ストアです。ネイティブ
...
3. 「フォルダーを追加…」またはドラッグ＆ドロップで調べたい場所を追加します。
   ...
   にします。
4. 「2. 重複を検索」を押します。スキャンだけでファイルが移動・削除されることは
   ありません。

![日本語表示のメイン画面](docs/images/ja/main-window.png)

結果では、各グループの先頭行が残すファイルです。余分なコピーだけを
...
必要な場合に CLI から永続ローカルカタログを明示的に作成・照会します。GUI の
完全一致「内容」スキャンがこのカタログを自動作成または更新することはありません。
